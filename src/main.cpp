...
void drawDashboard()
{
  ...
  int x = 5; // Initial starting point (X)
  int y = 22; // Initial Starting Point (Y)
  ...
  if ((shtPresent)&&(qmpPresent)) {
    ...
    canvas.setCursor(x, y);
    canvas.printf("Temp 2: %.1f%sF", qmpTempF, degreeSymbol);
    y += lineHeight;
...
void setup()
{
  ...
  canvas.fillScreen(TFT_BLACK);
  canvas.drawCenterString("M5Stack Core2", display.width() / 2, display.height() / 2 - 60);
  canvas.drawCenterString("Starting...", display.width() / 2+10, display.height() / 2 + 10);
  canvas.pushSprite(0, 0);
  delay(1000);
